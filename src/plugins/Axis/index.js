...
class Axis extends Base {
  ...
  get renderAttrs() {
    ...
    if (
      attrs.axisGap === undefined &&
      targetVisual &&
      targetVisual.constructor.name === "Bar"
    ) {
      //如果axisGap没有赋值
      //如果是柱状图
      attrs.axisGap = true;
  ...
  render(axis) {
    ...
    let arrOrient = this.chart.plugins
      ...
      .filter(Boolean);
    let nameStyle = getStyle(this, "name", [
      styles.name || {},
      filterClone(axis.nameAttr, [], ["pos"]),
    ]);
    let oldPos =
      (oldAxis.nameAttr && oldAxis.nameAttr.pos) || axis.nameAttr.pos;
    ...
    return (
      <Group ref="wrap" pos={[clientRect.left, clientRect.top]}>
        ...
        <Group>
          {axis.grids.map((grid, ind) => {
            ...
            if (orient === "left" || orient === "right") {
              if (
                (ind === 0 && arrOrient.includes("bottom")) ||
                (ind = axis.grids.length - 1 && arrOrient.includes("top"))
              ) {
                return <Node />;
              }
            } else if (orient === "top" || orient === "bottom") {
              if (
                (ind === 0 && arrOrient.includes("left")) ||
                (ind = axis.grids.length - 1 && arrOrient.includes("right"))
              ) {
                return <Node />;
              }
            }
            return style === false ? (
              ...
              <Polyline {...style} points={ani.from.points} animation={ani} />
            );
          })}
        ...
        {nameStyle === false || name === undefined || name.length === 0 ? (
          <Node />
        ) : (
          <Label {...nameStyle} text={name} animation={nameAni} />
        )}
      </Group>
    );
  }
