...
var Legend = /*#__PURE__*/function (_Base) {
  ...
  function Legend(attrs) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Legend).call(this, attrs));
  ...
  _createClass(Legend, [{
    ...
    value: function defaultAttrs() {
      ...
      return _ref = {
        ...
        innerGap: 4,
        lineGap: 5
      }, _defineProperty(_ref, "formatter", function formatter(d) {
        return d.text || d;
      }), _defineProperty(_ref, "padding", 2), _ref;
    }
  }, {
    key: "beforeRender",
    ...
    value: function reset() {
      ...
      var _this$renderAttrs2 = this.renderAttrs,
          ...
          lineGap = _this$renderAttrs2.lineGap,
          padding = _this$renderAttrs2.padding;
      ...
      this.arrLayout = this.arrLayout.map(function (item, index) {
        ...
        var iconPos = _this3.isVertical ? [lineWidth, legendsSize[_this3.lineCounter][1] + (textRect.height - iconSize[1]) / 2] : [legendsSize[_this3.lineCounter][0], (textRect.height - iconSize[1]) / 2 + _this3.lineCounter * (heightSize + lineGap)]; // 超过单行长度后换行

        if (_this3.isVertical && iconPos[1] + textRect.height > canvasHeight) {
          _this3.lineCounter++;
          iconPos = [iconPos[0] + legendsSize[_this3.lineCounter - 1][0], padding];
          maxTextWidth = 0; // 换行后重置最宽legend

          legendsSize.push([0, 0]);
        } else if (!_this3.isVertical && iconPos[0] + iconSize[0] + textRect.width + outGap > canvasWidth) {
          _this3.lineCounter++;
        ...
        var textAttrs = _objectSpread({}, item.textAttrs, {
          pos: _this3.isVertical ? [iconSize[0], 0] : [iconSize[0], 0],
      ...
      this.state.groupSize = this.isVertical ? [legendsSize.reduce(function (i, j) {
        return i + j[0];
      }, 0), this.lineCounter === 0 ? legendsSize[0][1] : canvasHeight] : [this.lineCounter === 0 ? legendsSize[0][0] : canvasWidth, (this.lineCounter + 1) * legendsSize[0][1]];
    } // changePage(e, el) {
