...
function getItemAttrs(defaultAttrs, orient, cell, value, scaleF, clientRect, axisGap) {
  ...
  if (orient === 'left' || orient === 'right') {
    ...
    var yv = scaleF(value);
    res.scaleAttr = _objectSpread({}, defaultAttrs[orient].scale, {
      pos: [x, height - yv]
    });
    res.gridAttr = _objectSpread({}, defaultAttrs[orient].grid, {
      points: [[x, height - yv], [width, height - yv]]
    });
    res.labelAttr = _objectSpread({}, defaultAttrs[orient].label, {
      width: left,
      text: '' + cell.text,
