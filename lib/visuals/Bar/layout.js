  ...
  if (!stack) {
    ...
    for (var i = 0, len = GROUP_NUM; i < len; i++) {
      ...
      for (var j = 0, lenj = data.length; j < lenj; j++) {
        ...
        var pos = transpose ? [tableSize.value * (1 - POSITIVE_RATIO), gap / 2 + (barWidth + groupGap) * (j - flag) + (barWidth * GROUP_BAR_NUM + groupGap * (GROUP_BAR_NUM - 1) + gap) * i] : [gap / 2 + (barWidth + groupGap) * (j - flag) + (barWidth * GROUP_BAR_NUM + groupGap * (GROUP_BAR_NUM - 1) + gap) * i, tableSize.value * POSITIVE_RATIO];
      ...
      var groupPos = transpose ? [0, (gap + barWidth * GROUP_BAR_NUM + groupGap * (GROUP_BAR_NUM - 1)) * i] : [(gap + barWidth * GROUP_BAR_NUM + groupGap * (GROUP_BAR_NUM - 1)) * i, 0];
    ...
    for (var _i = 0, _len = GROUP_NUM; _i < _len; _i++) {
      ...
      for (var _j = 0, _lenj = data.length; _j < _lenj; _j++) {
        ...
        var _pos = transpose ? [posX, gap / 2 + (barWidth + gap) * _i] : [gap / 2 + (barWidth + gap) * _i, posY];
      ...
      var _groupPos = transpose ? [0, (gap + barWidth) * _i] : [(gap + barWidth) * _i, 0];
