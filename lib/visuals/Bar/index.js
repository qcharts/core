...
var Bar = /*#__PURE__*/function (_Base) {
  ...
  function Bar(attrs) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Bar).call(this, attrs));

    _defineProperty(_assertThisInitialized(_this), "onMousemove", throttle(function (event, el) {
      if (_this.groups.length && !isNaN(event.x) && !isNaN(event.y)) {
        var bgpillarState = _this.bgpillarState;
        var curInd = 0;
  ...
  _createClass(Bar, [{
    ...
    value: function render(data) {
      ...
      return qcharts.h(Group, {
        ...
        return qcharts.h(Polyline, _extends({}, pillar.attrs, pillar.from, style, {
          animation: {
            from: pillar.from,
            to: pillar.to
          }
        ...
        var textStyle = getStyle(_this2, "text", [{
          barAttrs: barAttrs
        }, styles.text], [_this2.dataset.rows[ind % renderData.length][Math.floor(ind / renderData.length)].data, Math.floor(ind / renderData.length), ind % renderData.length]); //console.log('aaa', textStyle, styles.text)
