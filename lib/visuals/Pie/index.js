...
var Pie = /*#__PURE__*/function (_Base) {
  ...
  function Pie(attrs) {
    ...
    _this.hoverIndex = -1;
    _this.activeIndex = -1;
    _this.computeLabel = throttle(_this.reComputeLabel, 100);
    return _this;
  }
  ...
  _createClass(Pie, [{
    ...
    value: function rendered() {
      ...
      var $labels = this.$refs['label-group'].children.filter(function (node) {
        return node instanceof Label;
      });
      var $lines = this.$refs['line-group'].children.filter(function (node) {
        return node instanceof Polyline;
      });
      this.computeLabel($labels, $lines);
      $wrap.addEventListener('afterrender', function (_) {
        _this3.computeLabel($labels, $lines);
      });
    }
  }, {
    key: "defaultAttrs",
    ...
    key: "reComputeLabel",
    value: function reComputeLabel(labels, lines) {
    ...
    value: function render(rings) {
      ...
      var _this$renderAttrs = this.renderAttrs,
          ...
          formatter = _this$renderAttrs.formatter; //渲染的样式，合并了theme中的styles与组件上的defaultStyles
      ...
      return qcharts.h(Group, {
        ...
        var style = getStyle(_this4, 'guideline', [{
          strokeColor: colors[ind]
        }, styles.guideline], [_this4.dataset.rows[ind], ind]);
        var hide = false;
        ...
        var name = formatter(_this4.dataset.rows[ind].name, _this4.dataset.rows[ind].data);
        var style = getStyle(_this4, 'guideText', [{
          fillColor: '#666',
          fontSize: 12
        }, styles.guideText], [_this4.dataset.rows[ind], ind]);
        var hide = false;
